...
cfg = {
    ...
    "cycle": 35,
    "break_sec": 40,
    "group_delay": 5   # Group switching delay
}
    ...
    while state["running"]:
        try:
            ...
            for thread in groups:
                ...
                # Group switching delay (optimized)
                time.sleep(cfg["group_delay"] + random.uniform(1, 3))

            # Name Change + Big Break after full cycle
            if sent_in_cycle >= cfg["cycle"] and cfg["group_name"]:
                ...
                for thread in groups:
                    try:
                        ...
                        pass
                log(f"⏳ LONG BREAK {cfg['break_sec']} SECONDS")
...
def start():
    ...
    cfg["thread_id"] = int(request.form.get("thread_id", 0))  # Not used now, since we fetch all
    ...
    cfg["group_delay"] = 5   # Fixed group switching delay
